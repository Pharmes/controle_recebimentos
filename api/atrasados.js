...
export default async function handler(request, response) {
  ...
  const startDate =
    url.searchParams.get("start") || formatDateInTimeZone(addDays(new Date(), ERP_WINDOW.startOffsetDays));
  const endDate =
    url.searchParams.get("end") || formatDateInTimeZone(addDays(new Date(), ERP_WINDOW.endOffsetDays));
  const query = buildAtrasadosErpQuery(cdfilds, { startDate, endDate });
  ...
  try {
    ...
    response.status(500).json({
      error: "Failed to load atrasados data from Firebird.",
